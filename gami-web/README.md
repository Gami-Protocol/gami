...
| Swap / Bridge | Uniswap Trading API | `check_approval` → `quote` → `swap` or `/order`; bridge ETH/ARB/OP/POL → Base |
| Destination | USDC on Base or `$GAMI` | USDC funds the sale; `$GAMI` needs `VITE_GAMI_TOKEN_ADDRESS` |
| Optional overrides | `VITE_FIAT_ONRAMP_URL`, `VITE_USDT_SWAP_URL` | Support `{wallet}`, `{amount}`, `{usdc}` |

Set `VITE_UNISWAP_API_KEY` from the [Uniswap Developer Portal](https://developers.uniswap.org/) to enable in-app swaps and bridges onto the Gami chain (Base).
