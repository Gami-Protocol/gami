...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const referralLink =
      ...
      (referralCode ? `https://gamiprotocol.io/?ref=${encodeURIComponent(referralCode)}` : 'https://gamiprotocol.io/waitlist');
    ...
    if (!apiKey) {
      return json({ ok: true, skipped: true, reason: 'RESEND_API_KEY not set' });
    }

    const resend = new Resend(apiKey);
    const from = Deno.env.get('WAITLIST_ALERT_FROM') || 'Gami Protocol <onboarding@resend.dev>';
